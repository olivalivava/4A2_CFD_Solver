...
!     INSERT
!     ****************
      real :: l_i_lc(g%nj), v_guess(g%ni), ro_guess(g%ni), t_guess(g%ni), M_guess(g%ni)
      real :: p_guess(g%ni)
      real :: R, gamma, M_out, p0, t0, mass_out, p_out, cp, cv
      real :: mach_lim, t_lim, t_new, tol, t_max, E_tot
...
!         Store the exit density and internal energy as if they were uniform (upper two graphs have no change with displacement
          g%ro = ro_out 
...
!         mass_out = (gamma*M_out*l_i(nj-1)*p_out)/(sqrt((gamma-1)*cp*t0))*((1+(gamma-1)*M_out*M_out/2)**(0.5))
!         write(6,*) "mass_out = ", mass_out
...
!         if (M_out > mach_lim) then
!            write(6,*) "ERROR: Mach number in the intital guess greater than the allowrance."
!            stop
!         end if
...
!         *******************
         ...
         do i=1, nj-1
            ...
            M_guess(i) = v_guess(i)/sqrt(gamma*R*t_guess(i))
            p_guess(i) = p0/((1+0.5*(gamma-1)*(M_guess(i)**2))**(gamma/(gamma-1)))
            ro_guess(i) = p_guess(i) / (R*t_guess(i))
            v_guess(i) = mass_out/(l_i(i)*ro_guess(i))
         end do

         t_max = maxval(t_guess, dim=1)
         if (t_max > t_lim) then
...
!         ********************
          do i=1, ni-1
             g%ro(i,:) = ro_guess(i)
             g%roe(i,:) = g%ro(i,:) * (cv*t_guess(i) + 0.5 * (v_guess(i)**2))
          end do
          j_mid = nj / 2        
          do i = 1,ni-1
              lx = g%lx_j(i,j_mid); ly = g%ly_j(i,j_mid); 
              l = hypot(lx,ly)
              print *, i,j_mid,lx,ly,l
